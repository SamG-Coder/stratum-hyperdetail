// CUDA WebShader 0.1.0. Generated from kernel buildBVH.
@group(0) @binding(0) var<storage, read> b_Queue: array<i32>;
@group(0) @binding(1) var<storage, read_write> b_Nodes: array<f32>;
struct CWParams {
  p_first: i32,
  cw_pad_4: u32,
  cw_pad_8: u32,
...
fn main(
  ...
  var v_q: i32 = (v_i / cw_params.p_first);
  var v_j: i32 = ((v_i % cw_params.p_first) + cw_params.p_first);
  if ((v_q >= b_Queue[0i])) {
    return;
  }
  var v_slot: i32 = b_Queue[(v_q + 1i)];
  var v_n: i32 = (((v_slot * 4096i) + v_j) * 8i);
  var v_a: i32 = (((v_slot * 4096i) + (v_j * 2i)) * 8i);
  var v_b: i32 = (v_a + 8i);
  {
    var v_k: i32 = 0i;
    loop {
      if (!(v_k < 3i)) { break; }
      let cw_argument_index_7 = (v_a + v_k);
      let cw_argument_index_8 = (v_b + v_k);
      b_Nodes[(v_n + v_k)] = min(b_Nodes[cw_argument_index_7], b_Nodes[cw_argument_index_8]);
      let cw_argument_index_9 = ((v_a + 4i) + v_k);
      let cw_argument_index_10 = ((v_b + 4i) + v_k);
      b_Nodes[((v_n + 4i) + v_k)] = max(b_Nodes[cw_argument_index_9], b_Nodes[cw_argument_index_10]);
      continuing {
        v_k += i32(1);
      }
    }
  }
  b_Nodes[(v_n + 3i)] = (b_Nodes[(v_a + 3i)] + b_Nodes[(v_b + 3i)]);
  b_Nodes[(v_n + 7i)] = 0.0f;
}
