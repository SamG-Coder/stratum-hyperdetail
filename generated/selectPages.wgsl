// CUDA WebShader 0.1.0. Generated from kernel selectPages.
@group(0) @binding(0) var<storage, read> b_C: array<f32>;
@group(0) @binding(1) var<storage, read> b_World: array<f32>;
@group(0) @binding(2) var<storage, read> b_Meta: array<f32>;
@group(0) @binding(3) var<storage, read_write> b_Req: array<f32>;
...
fn f_cw_buffer_helper_0(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  let cw_argument_index_11 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_12 = (cw_buffer_offset_0 + 4i);
  let cw_argument_index_13 = (cw_buffer_offset_0 + 4i);
  let cw_argument_index_14 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_15 = (cw_buffer_offset_0 + 4i);
  return vec3<f32>((sin(b_C[cw_argument_index_11]) * cos(b_C[cw_argument_index_12])), sin(b_C[cw_argument_index_13]), (cos(b_C[cw_argument_index_14]) * cos(b_C[cw_argument_index_15])));
}
fn f_cw_buffer_helper_1(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  let cw_argument_index_16 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_19 = (cw_buffer_offset_0 + 3i);
  return vec3<f32>(cos(b_C[cw_argument_index_16]), 0.0f, (-sin(b_C[cw_argument_index_19])));
}
...
fn main(
  ...
  var v_s: i32 = i32(((cw_block.x * cw_block_size.x) + cw_thread.x));
  if ((v_s >= 256i)) {
    return;
  }
  var v_b: i32 = (v_s * 8i);
  var v_ax: i32 = (i32(floor(cw_divide_f32((b_C[0i] + (36.0f * 0.5f)), 36.0f))) - (16i / 2i));
  var v_az: i32 = (i32(floor(cw_divide_f32((b_C[2i] + (36.0f * 0.5f)), 36.0f))) - (16i / 2i));
  var v_cx: i32 = (v_ax + f_imod(((v_s % 16i) - f_imod(v_ax, 16i, cw_thread, cw_block, cw_grid)), 16i, cw_thread, cw_block, cw_grid));
  var v_cz: i32 = (v_az + f_imod(((v_s / 16i) - f_imod(v_az, 16i, cw_thread, cw_block, cw_grid)), 16i, cw_thread, cw_block, cw_grid));
  b_Req[v_b] = f32(v_cx);
  b_Req[(v_b + 1i)] = f32(v_cz);
  b_Req[(v_b + 2i)] = 0.0f;
  b_Req[(v_b + 3i)] = 0.0f;
  if ((!f_inCity(v_cx, v_cz, cw_thread, cw_block, cw_grid))) {
    return;
  }
  var v_w: i32 = (f_worldIndex(v_cx, v_cz, cw_thread, cw_block, cw_grid) * 8i);
  var v_h: f32 = b_World[(v_w + 2i)];
  var v_delta: vec3<f32> = vec3<f32>(((f32(v_cx) * 36.0f) - b_C[0i]), ((v_h * 0.5f) - b_C[1i]), ((f32(v_cz) * 36.0f) - b_C[2i]));
  var v_dist: f32 = max(1.0f, (f_length3(v_delta, cw_thread, cw_block, cw_grid) - 23.0f));
  var v_z: f32 = f_dot3(v_delta, f_cw_buffer_helper_0(0i, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  var v_xx: f32 = abs(f_dot3(v_delta, f_cw_buffer_helper_1(0i, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid));
  var v_yy: f32 = abs(f_dot3(v_delta, f_cross3(f_cw_buffer_helper_0(0i, cw_thread, cw_block, cw_grid), f_cw_buffer_helper_1(0i, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid));
  var cw_tmp_7: f32;
  if ((b_C[20i] > 0.1f)) {
    cw_tmp_7 = b_C[20i];
  } else {
  ...
  var v_aspect: f32 = cw_tmp_7;
  var v_visible: bool = (((((v_z + 45.0f) > 0.0f) && (v_xx < (((v_z * 0.54f) * v_aspect) + 65.0f))) && (v_yy < ((v_z * 0.54f) + 65.0f))) || (v_dist < 55.0f));
  if (((!v_visible) || (b_C[17i] > 0.5f))) {
    return;
  }
  var v_ppm: f32 = cw_divide_f32(b_C[13i], (1.08f * v_dist));
  var cw_tmp_9: i32;
  if ((v_ppm > 6.5f)) {
    cw_tmp_9 = 3i;
  } else {
  ...
  var v_lod: i32 = cw_tmp_9;
  var v_m: i32 = (v_s * 12i);
  var v_same: bool = (((b_Meta[(v_m + 3i)] > 0.5f) && (i32(b_Meta[v_m]) == v_cx)) && (i32(b_Meta[(v_m + 1i)]) == v_cz));
  var v_old: i32 = i32(b_Meta[(v_m + 2i)]);
  if (((v_same && (v_old == 3i)) && (v_ppm > 5.3f))) {
    v_lod = 3i;
  }
  if ((((v_same && (v_old == 2i)) && (v_ppm > 1.2f)) && (v_ppm < 7.2f))) {
    v_lod = 2i;
  }
  b_Req[(v_b + 2i)] = f32(v_lod);
  b_Req[(v_b + 4i)] = v_ppm;
  if (((!v_same) || (v_old != v_lod))) {
    var cw_tmp_10: f32;
    if (v_same) {
      cw_tmp_10 = 0.0f;
    } else {
    ...
    b_Req[(v_b + 3i)] = (cw_divide_f32(10000.0f, (v_dist + 5.0f)) + cw_tmp_10);
  }
}
