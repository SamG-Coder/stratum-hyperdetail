// CUDA WebShader 0.1.0. Generated from kernel resolveFrame.
...
@group(0) @binding(4) var<storage, read> b_Stats: array<f32>;
@group(0) @binding(5) var<storage, read> b_Hit: array<f32>;
@group(0) @binding(6) var<storage, read> b_P: array<f32>;
struct CWParams {
  p_width: i32,
  p_height: i32,
  cw_pad_8: u32,
...
fn f_cw_buffer_helper_0(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  let cw_argument_index_27 = (cw_buffer_offset_0 + 0i);
  let cw_argument_index_28 = (cw_buffer_offset_0 + 1i);
  let cw_argument_index_29 = (cw_buffer_offset_0 + 2i);
  return vec3<f32>(b_C[cw_argument_index_27], b_C[cw_argument_index_28], b_C[cw_argument_index_29]);
}
...
fn f_cw_buffer_helper_2(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_point: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  ...
  let cw_argument_index_30 = (cw_buffer_offset_0 + v_b);
  let cw_argument_index_31 = (cw_buffer_offset_0 + (v_b + 1i));
  let cw_argument_index_32 = (cw_buffer_offset_0 + (v_b + 2i));
  var v_p: vec3<f32> = f_localPoint((v_point - vec3<f32>(b_P[cw_argument_index_30], b_P[cw_argument_index_31], b_P[cw_argument_index_32])), v_turn, cw_thread, cw_block, cw_grid);
  let cw_argument_index_33 = (cw_buffer_offset_0 + (v_b + 4i));
  let cw_argument_index_34 = (cw_buffer_offset_0 + (v_b + 5i));
  let cw_argument_index_35 = (cw_buffer_offset_0 + (v_b + 6i));
  var v_h: vec3<f32> = vec3<f32>(b_P[cw_argument_index_33], b_P[cw_argument_index_34], b_P[cw_argument_index_35]);
  ...
  if ((v_shape == 1i)) {
    ...
    if ((v_shape == 2i)) {
      if ((abs((abs(v_p.y) - v_h.y)) < 0.01f)) {
        ...
        v_n = vec3<f32>(0.0f, cw_tmp_36, 0.0f);
      } else {
      ...
      if (((v_shape == 4i) || (v_shape == 7i))) {
        if ((abs((abs(v_p.z) - v_h.z)) < 0.008f)) {
          ...
          v_n = vec3<f32>(0.0f, 0.0f, cw_tmp_37);
        } else {
          ...
          v_n = (f_norm3(vec3<f32>(cw_divide_f32(v_p.x, (v_h.x * v_h.x)), cw_divide_f32(v_p.y, (v_h.y * v_h.y)), 0.0f), cw_thread, cw_block, cw_grid) * vec3<f32>(cw_tmp_38));
        }
      } else {
        if ((v_shape == 3i)) {
          if ((abs((abs(v_p.z) - v_h.z)) < 0.006f)) {
            ...
            v_n = vec3<f32>(0.0f, 0.0f, cw_tmp_39);
          } else {
            if ((v_p.y < 0.003f)) {
              ...
              var cw_tmp_40: f32;
              if ((v_p.x < 0.0f)) {
                cw_tmp_40 = cw_divide_f32((-v_h.y), v_h.x);
              } else {
                cw_tmp_40 = cw_divide_f32(v_h.y, v_h.x);
              }
              v_n = f_norm3(vec3<f32>(cw_tmp_40, 1.0f, 0.0f), cw_thread, cw_block, cw_grid);
            }
          }
        } else {
          var v_a: vec3<f32> = vec3<f32>(abs((abs(v_p.x) - v_h.x)), abs((abs(v_p.y) - v_h.y)), abs((abs(v_p.z) - v_h.z)));
          var cw_tmp_45: vec3<f32>;
          if (((v_a.x < v_a.y) && (v_a.x < v_a.z))) {
            ...
            cw_tmp_45 = vec3<f32>(cw_tmp_41, 0.0f, 0.0f);
          } else {
            var cw_tmp_44: vec3<f32>;
            if ((v_a.y < v_a.z)) {
              var cw_tmp_42: f32;
              if ((v_p.y < 0.0f)) {
                cw_tmp_42 = (-1.0f);
              ...
              cw_tmp_44 = vec3<f32>(0.0f, cw_tmp_42, 0.0f);
            } else {
              var cw_tmp_43: f32;
              if ((v_p.z < 0.0f)) {
                cw_tmp_43 = (-1.0f);
              ...
              cw_tmp_44 = vec3<f32>(0.0f, 0.0f, cw_tmp_43);
            }
            cw_tmp_45 = cw_tmp_44;
          }
          v_n = cw_tmp_45;
          ...
          var v_q: vec3<f32> = vec3<f32>((max(((abs(v_p.x) - v_h.x) + v_bevel), 0.0f) * cw_tmp_46), (max(((abs(v_p.y) - v_h.y) + v_bevel), 0.0f) * cw_tmp_47), (max(((abs(v_p.z) - v_h.z) + v_bevel), 0.0f) * cw_tmp_48));
...
fn f_cw_buffer_helper_3(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  let cw_argument_index_49 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_50 = (cw_buffer_offset_0 + 4i);
  let cw_argument_index_51 = (cw_buffer_offset_0 + 4i);
  let cw_argument_index_52 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_53 = (cw_buffer_offset_0 + 4i);
  return vec3<f32>((sin(b_C[cw_argument_index_49]) * cos(b_C[cw_argument_index_50])), sin(b_C[cw_argument_index_51]), (cos(b_C[cw_argument_index_52]) * cos(b_C[cw_argument_index_53])));
}
fn f_cw_buffer_helper_4(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  let cw_argument_index_54 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_57 = (cw_buffer_offset_0 + 3i);
  return vec3<f32>(cos(b_C[cw_argument_index_54]), 0.0f, (-sin(b_C[cw_argument_index_57])));
}
...
fn main(
  ...
  let cw_argument_index_18 = v_b;
  let cw_argument_index_19 = (v_b + 1i);
  let cw_argument_index_20 = (v_b + 2i);
  var v_c: vec3<f32> = vec3<f32>(b_Linear[cw_argument_index_18], b_Linear[cw_argument_index_19], b_Linear[cw_argument_index_20]);
  if ((((b_C[11i] < 0.5f) && (i32(b_Hit[(v_b + 1i)]) != (-10000i))) && (b_Hit[v_b] < 250.0f))) {
    var v_ro: vec3<f32> = f_cw_buffer_helper_0(0i, cw_thread, cw_block, cw_grid);
    var v_point: vec3<f32> = (v_ro + (f_cw_buffer_helper_1(0i, v_x, v_y, cw_params.p_width, cw_params.p_height, cw_thread, cw_block, cw_grid) * vec3<f32>(b_Hit[v_b])));
    var v_id: i32 = i32(b_Hit[(v_b + 1i)]);
    var cw_tmp_21: vec3<f32>;
    if ((v_id >= 0i)) {
      cw_tmp_21 = f_cw_buffer_helper_2(0i, v_id, v_point, cw_thread, cw_block, cw_grid);
    } else {
      cw_tmp_21 = vec3<f32>(0.0f, 1.0f, 0.0f);
    }
    var v_n: vec3<f32> = cw_tmp_21;
    var v_occlusion: f32 = 0.0f;
    let cw_argument_index_22 = v_b;
    var v_radius: f32 = f_clampf((cw_divide_f32(f32(cw_params.p_height), (max(b_Hit[cw_argument_index_22], 0.5f) * 1.08f)) * 0.8f), 3.0f, 28.0f, cw_thread, cw_block, cw_grid);
    {
      var v_k: i32 = 0i;
      loop {
        if (!(v_k < 8i)) { break; }
        var v_a: f32 = (((f32(v_k) * 3.141592653589793f) * 0.25f) + 0.2f);
        var v_xx: i32 = i32(f_clampf((f32(v_x) + (cos(v_a) * v_radius)), 0.0f, f32((cw_params.p_width - 1i)), cw_thread, cw_block, cw_grid));
        var v_yy: i32 = i32(f_clampf((f32(v_y) + (sin(v_a) * v_radius)), 0.0f, f32((cw_params.p_height - 1i)), cw_thread, cw_block, cw_grid));
        var v_j: i32 = (((v_yy * cw_params.p_width) + v_xx) * 4i);
        if ((i32(b_Hit[(v_j + 1i)]) != (-10000i))) {
          var v_other: vec3<f32> = (v_ro + (f_cw_buffer_helper_1(0i, v_xx, v_yy, cw_params.p_width, cw_params.p_height, cw_thread, cw_block, cw_grid) * vec3<f32>(b_Hit[v_j])));
          var v_delta: vec3<f32> = (v_other - v_point);
          var v_len: f32 = f_length3(v_delta, cw_thread, cw_block, cw_grid);
          if (((v_len > 0.04f) && (v_len < 2.0f))) {
            var v_elevated: f32 = (f_dot3(v_n, (v_delta / vec3<f32>(v_len)), cw_thread, cw_block, cw_grid) - 0.12f);
            v_occlusion = (v_occlusion + ((max(0.0f, v_elevated) * exp(((-v_len) * 1.4f))) * 0.28f));
          }
        }
        continuing {
          v_k += i32(1);
    ...
    v_c = (v_c * vec3<f32>(f_clampf((1.0f - v_occlusion), 0.55f, 1.0f, cw_thread, cw_block, cw_grid)));
  }
  var v_bloom: vec3<f32> = vec3<f32>(0.0f, 0.0f, 0.0f);
  if ((b_C[11i] < 0.5f)) {
    {
      var v_k: i32 = 0i;
      loop {
        if (!(v_k < 8i)) { break; }
        var v_a: f32 = cw_divide_f32((f32(v_k) * 3.141592653589793f), 4.0f);
        var v_xx: i32 = i32(f_clampf((f32(v_x) + (cos(v_a) * 4.0f)), 0.0f, f32((cw_params.p_width - 1i)), cw_thread, cw_block, cw_grid));
        var v_yy: i32 = i32(f_clampf((f32(v_y) + (sin(v_a) * 4.0f)), 0.0f, f32((cw_params.p_height - 1i)), cw_thread, cw_block, cw_grid));
        var v_j: i32 = (((v_yy * cw_params.p_width) + v_xx) * 4i);
        v_bloom = (v_bloom + (vec3<f32>(max(0.0f, (b_Linear[v_j] - 1.0f)), max(0.0f, (b_Linear[(v_j + 1i)] - 1.0f)), max(0.0f, (b_Linear[(v_j + 2i)] - 1.0f))) * vec3<f32>(0.022f)));
        continuing {
          v_k += i32(1);
        }
      }
  ...
  v_c = (v_c + v_bloom);
  var cw_tmp_23: f32;
  if (((b_C[15i] > 0.5f) || (b_Stats[1i] > 0.0f))) {
    cw_tmp_23 = 1.0f;
  } else {
    cw_tmp_23 = min(32.0f, (b_History[(v_b + 3i)] + 1.0f));
  }
  var v_count: f32 = cw_tmp_23;
  let cw_argument_index_24 = v_b;
  let cw_argument_index_25 = (v_b + 1i);
  let cw_argument_index_26 = (v_b + 2i);
  var v_old: vec3<f32> = vec3<f32>(b_History[cw_argument_index_24], b_History[cw_argument_index_25], b_History[cw_argument_index_26]);
  v_c = f_mix3(v_old, v_c, cw_divide_f32(1.0f, v_count), cw_thread, cw_block, cw_grid);
  b_History[v_b] = v_c.x;
  b_History[(v_b + 1i)] = v_c.y;
  b_History[(v_b + 2i)] = v_c.z;
  b_History[(v_b + 3i)] = v_count;
  var v_sx: f32 = ((cw_divide_f32(f32(v_x), f32(cw_params.p_width)) - 0.5f) * 1.3f);
  var v_sy: f32 = ((cw_divide_f32(f32(v_y), f32(cw_params.p_height)) - 0.5f) * 1.3f);
  var v_vignette: f32 = (1.0f - (((v_sx * v_sx) + (v_sy * v_sy)) * 0.18f));
  v_c = (v_c * vec3<f32>((b_C[9i] * v_vignette)));
  v_c = vec3<f32>(cw_pow_f32(f_aces(v_c.x, cw_thread, cw_block, cw_grid), cw_divide_f32(1.0f, 2.2f)), cw_pow_f32(f_aces(v_c.y, cw_thread, cw_block, cw_grid), cw_divide_f32(1.0f, 2.2f)), cw_pow_f32(f_aces(v_c.z, cw_thread, cw_block, cw_grid), cw_divide_f32(1.0f, 2.2f)));
  b_Pixels[((v_y * cw_params.p_width) + v_x)] = f_packRGBA(v_c, cw_thread, cw_block, cw_grid);
}
