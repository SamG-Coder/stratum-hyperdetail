// CUDA WebShader 0.1.0. Generated from kernel prepareMaterialBatch.
@group(0) @binding(0) var<storage, read> b_P: array<f32>;
@group(0) @binding(1) var<storage, read> b_Hit: array<f32>;
@group(0) @binding(2) var<storage, read> b_C: array<f32>;
@group(0) @binding(3) var<storage, read_write> b_Work: array<f32>;
struct CWParams {
  p_width: i32,
  p_height: i32,
  cw_pad_8: u32,
...
fn f_cw_buffer_helper_2(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  let cw_argument_index_18 = (cw_buffer_offset_0 + 0i);
  let cw_argument_index_19 = (cw_buffer_offset_0 + 1i);
  let cw_argument_index_20 = (cw_buffer_offset_0 + 2i);
  return vec3<f32>(b_C[cw_argument_index_18], b_C[cw_argument_index_19], b_C[cw_argument_index_20]);
}
fn f_cw_buffer_helper_3(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_point: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  let cw_argument_index_21 = (cw_buffer_offset_0 + v_b);
  let cw_argument_index_22 = (cw_buffer_offset_0 + (v_b + 1i));
  let cw_argument_index_23 = (cw_buffer_offset_0 + (v_b + 2i));
  var v_p: vec3<f32> = f_localPoint((v_point - vec3<f32>(b_P[cw_argument_index_21], b_P[cw_argument_index_22], b_P[cw_argument_index_23])), v_turn, cw_thread, cw_block, cw_grid);
  let cw_argument_index_24 = (cw_buffer_offset_0 + (v_b + 4i));
  let cw_argument_index_25 = (cw_buffer_offset_0 + (v_b + 5i));
  let cw_argument_index_26 = (cw_buffer_offset_0 + (v_b + 6i));
  var v_h: vec3<f32> = vec3<f32>(b_P[cw_argument_index_24], b_P[cw_argument_index_25], b_P[cw_argument_index_26]);
  ...
  if ((v_shape == 1i)) {
    ...
    if ((v_shape == 2i)) {
      if ((abs((abs(v_p.y) - v_h.y)) < 0.01f)) {
        ...
        v_n = vec3<f32>(0.0f, cw_tmp_27, 0.0f);
      } else {
      ...
      if (((v_shape == 4i) || (v_shape == 7i))) {
        if ((abs((abs(v_p.z) - v_h.z)) < 0.008f)) {
          ...
          v_n = vec3<f32>(0.0f, 0.0f, cw_tmp_28);
        } else {
          ...
          v_n = (f_norm3(vec3<f32>(cw_divide_f32(v_p.x, (v_h.x * v_h.x)), cw_divide_f32(v_p.y, (v_h.y * v_h.y)), 0.0f), cw_thread, cw_block, cw_grid) * vec3<f32>(cw_tmp_29));
        }
      } else {
        if ((v_shape == 3i)) {
          if ((abs((abs(v_p.z) - v_h.z)) < 0.006f)) {
            ...
            v_n = vec3<f32>(0.0f, 0.0f, cw_tmp_30);
          } else {
            if ((v_p.y < 0.003f)) {
              ...
              var cw_tmp_31: f32;
              if ((v_p.x < 0.0f)) {
                cw_tmp_31 = cw_divide_f32((-v_h.y), v_h.x);
              } else {
                cw_tmp_31 = cw_divide_f32(v_h.y, v_h.x);
              }
              v_n = f_norm3(vec3<f32>(cw_tmp_31, 1.0f, 0.0f), cw_thread, cw_block, cw_grid);
            }
          }
        } else {
          var v_a: vec3<f32> = vec3<f32>(abs((abs(v_p.x) - v_h.x)), abs((abs(v_p.y) - v_h.y)), abs((abs(v_p.z) - v_h.z)));
          var cw_tmp_36: vec3<f32>;
          if (((v_a.x < v_a.y) && (v_a.x < v_a.z))) {
            ...
            cw_tmp_36 = vec3<f32>(cw_tmp_32, 0.0f, 0.0f);
          } else {
            var cw_tmp_35: vec3<f32>;
            if ((v_a.y < v_a.z)) {
              var cw_tmp_33: f32;
              if ((v_p.y < 0.0f)) {
                cw_tmp_33 = (-1.0f);
              ...
              cw_tmp_35 = vec3<f32>(0.0f, cw_tmp_33, 0.0f);
            } else {
              var cw_tmp_34: f32;
              if ((v_p.z < 0.0f)) {
                cw_tmp_34 = (-1.0f);
              ...
              cw_tmp_35 = vec3<f32>(0.0f, 0.0f, cw_tmp_34);
            }
            cw_tmp_36 = cw_tmp_35;
          }
          v_n = cw_tmp_36;
          ...
          var v_q: vec3<f32> = vec3<f32>((max(((abs(v_p.x) - v_h.x) + v_bevel), 0.0f) * cw_tmp_37), (max(((abs(v_p.y) - v_h.y) + v_bevel), 0.0f) * cw_tmp_38), (max(((abs(v_p.z) - v_h.z) + v_bevel), 0.0f) * cw_tmp_39));
...
fn f_cw_buffer_helper_4(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  let cw_argument_index_40 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_41 = (cw_buffer_offset_0 + 4i);
  let cw_argument_index_42 = (cw_buffer_offset_0 + 4i);
  let cw_argument_index_43 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_44 = (cw_buffer_offset_0 + 4i);
  return vec3<f32>((sin(b_C[cw_argument_index_40]) * cos(b_C[cw_argument_index_41])), sin(b_C[cw_argument_index_42]), (cos(b_C[cw_argument_index_43]) * cos(b_C[cw_argument_index_44])));
}
fn f_cw_buffer_helper_5(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  let cw_argument_index_45 = (cw_buffer_offset_0 + 3i);
  let cw_argument_index_48 = (cw_buffer_offset_0 + 3i);
  return vec3<f32>(cos(b_C[cw_argument_index_45]), 0.0f, (-sin(b_C[cw_argument_index_48])));
}
...
fn main(
  ...
  var v_chosen: i32 = (-1i);
  {
    ...
    loop {
      if (!(v_trial < 6i)) { break; }
      var v_px: i32 = i32((f_hash1((((v_i * 1877i) + (v_trial * 331i)) + (i32(b_C[6i]) * 59i)), cw_thread, cw_block, cw_grid) * f32((cw_params.p_width * cw_params.p_height))));
      var v_id: i32 = i32(b_Hit[((v_px * 4i) + 1i)]);
      if ((v_id >= 0i)) {
        var v_mat: i32 = i32(b_P[((v_id * 16i) + 7i)]);
        if (((v_mat == 0i) || (v_mat == 1i))) {
          v_chosen = v_px;
        }
      }
  ...
  var v_id: i32 = i32(b_Hit[((v_chosen * 4i) + 1i)]);
  var v_mat: i32 = i32(b_P[((v_id * 16i) + 7i)]);
  var v_seed: f32 = b_P[((v_id * 16i) + 9i)];
  var v_rd: vec3<f32> = f_cw_buffer_helper_1(0i, (v_chosen % cw_params.p_width), (v_chosen / cw_params.p_width), cw_params.p_width, cw_params.p_height, cw_thread, cw_block, cw_grid);
  var v_p: vec3<f32> = (f_cw_buffer_helper_2(0i, cw_thread, cw_block, cw_grid) + (v_rd * vec3<f32>(b_Hit[(v_chosen * 4i)])));
  var v_n: vec3<f32> = f_cw_buffer_helper_3(0i, v_id, v_p, cw_thread, cw_block, cw_grid);
  var v_uv: vec2<f32> = f_masonryUV(v_p, v_n, cw_thread, cw_block, cw_grid);
  {
    var v_j: i32 = 0i;
    loop {
      if (!(v_j < 8i)) { break; }
      b_Work[(v_b + v_j)] = f_feature(v_j, v_uv.x, v_uv.y, v_seed, v_mat, cw_thread, cw_block, cw_grid);
      continuing {
        v_j += i32(1);
      }
    }
  }
  var v_target: vec3<f32> = f_substrate(v_uv.x, v_uv.y, v_seed, v_mat, cw_thread, cw_block, cw_grid);
  b_Work[(v_b + 8i)] = v_target.x;
  b_Work[(v_b + 9i)] = v_target.y;
  b_Work[(v_b + 10i)] = v_target.z;
  b_Work[(v_b + 62i)] = 1.0f;
  b_Work[(v_b + 63i)] = f32(v_mat);
}
