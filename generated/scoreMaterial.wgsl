// CUDA WebShader 0.1.0. Generated from kernel scoreMaterial.
@group(0) @binding(0) var<storage, read> b_C: array<f32>;
@group(0) @binding(1) var<storage, read> b_Work: array<f32>;
@group(0) @binding(2) var<storage, read_write> b_Brain: array<f32>;
...
fn main(
  ...
  if ((((cw_block.x != u32(0i)) || (cw_thread.x != u32(0i))) || (!f_cw_buffer_helper_0(0i, cw_thread, cw_block, cw_grid)))) {
    return;
  }
  var v_err: f32 = 0.0f;
  var v_baseline: f32 = 0.0f;
  var v_n: f32 = 0.0f;
  {
    var v_i: i32 = 56i;
    loop {
      if (!(v_i < 64i)) { break; }
      var v_b: i32 = (v_i * 64i);
      if ((b_Work[(v_b + 62i)] > 0.5f)) {
        v_n = (v_n + 1.0f);
        var v_mean: vec3<f32> = f_substrateMean(i32(b_Work[(v_b + 63i)]), cw_thread, cw_block, cw_grid);
        {
          var v_o: i32 = 0i;
          loop {
            if (!(v_o < 3i)) { break; }
            var v_d: f32 = (b_Work[((v_b + 11i) + v_o)] - b_Work[((v_b + 8i) + v_o)]);
            v_err = (v_err + (v_d * v_d));
            var cw_tmp_19: f32;
            if ((v_o == 0i)) {
              cw_tmp_19 = v_mean.x;
            ...
            var v_v: f32 = cw_tmp_19;
            var v_e: f32 = (v_v - b_Work[((v_b + 8i) + v_o)]);
            v_baseline = (v_baseline + (v_e * v_e));
            continuing {
              v_o += i32(1);
            }
          }
  ...
  if ((v_n > 0.0f)) {
    ...
    var v_a: f32 = cw_tmp_20;
    let cw_argument_index_21 = 2i;
    b_Brain[2i] = f_lerpf(b_Brain[cw_argument_index_21], cw_divide_f32(v_err, (v_n * 3.0f)), v_a, cw_thread, cw_block, cw_grid);
    let cw_argument_index_22 = 3i;
    b_Brain[3i] = f_lerpf(b_Brain[cw_argument_index_22], cw_divide_f32(v_baseline, (v_n * 3.0f)), v_a, cw_thread, cw_block, cw_grid);
    b_Brain[1i] = (b_Brain[1i] + v_n);
  }
  var cw_tmp_23: f32;
  if ((((b_Brain[1i] >= 256.0f) && (b_Brain[2i] < (b_Brain[3i] * 0.9f))) && (b_Brain[2i] < 0.0012f))) {
    cw_tmp_23 = 1.0f;
  } else {
    cw_tmp_23 = 0.0f;
  }
  b_Brain[4i] = cw_tmp_23;
}
