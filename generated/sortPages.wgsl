// CUDA WebShader 0.1.0. Generated from kernel sortPages.
@group(0) @binding(0) var<storage, read> b_Queue: array<i32>;
@group(0) @binding(1) var<storage, read> b_P: array<f32>;
@group(0) @binding(2) var<storage, read_write> b_Order: array<i32>;
@group(0) @binding(3) var<storage, read_write> b_Nodes: array<f32>;
const cw_block_size: vec3<u32> = vec3<u32>(256u, 1u, 1u);
var<workgroup> s_keys: array<u32, 2048>;
var<workgroup> s_ids: array<u32, 2048>;
...
fn main(
  ...
  var v_lane: i32 = i32(cw_thread.x);
  var v_valid: bool = (v_q < b_Queue[0i]);
  var v_slot: i32 = 0i;
  if (v_valid) {
    v_slot = b_Queue[(v_q + 1i)];
  }
  {
    var v_r: i32 = 0i;
    loop {
      if (!(v_r < (2048i / 256i))) { break; }
      var v_i: i32 = (v_lane + (v_r * 256i));
      var v_key: u32 = 4294967295u;
      if (v_valid) {
        var v_b: i32 = (((v_slot * 2048i) + v_i) * 16i);
        if ((b_P[(v_b + 3i)] >= 0.0f)) {
          var v_cx: f32 = (floor(cw_divide_f32((b_P[v_b] + (36.0f * 0.5f)), 36.0f)) * 36.0f);
          var v_cz: f32 = (floor(cw_divide_f32((b_P[(v_b + 2i)] + (36.0f * 0.5f)), 36.0f)) * 36.0f);
          v_key = f_morton3(cw_divide_f32(((b_P[v_b] - v_cx) + 18.0f), 36.0f), cw_divide_f32(b_P[(v_b + 1i)], 70.0f), cw_divide_f32(((b_P[(v_b + 2i)] - v_cz) + 18.0f), 36.0f), cw_thread, cw_block, cw_grid);
        }
      }
      s_keys[v_i] = v_key;
      s_ids[v_i] = u32(v_i);
  ...
  workgroupBarrier();
  {
    var v_size: i32 = 2i;
    loop {
      if (!(v_size <= 2048i)) { break; }
      {
        var v_stride: i32 = (v_size / 2i);
        loop {
          if (!(v_stride > 0i)) { break; }
          {
            ...
            loop {
              if (!(v_r < (2048i / 256i))) { break; }
              var v_i: i32 = (v_lane + (v_r * 256i));
              var v_other: i32 = (v_i ^ v_stride);
              if ((v_other > v_i)) {
                var v_ascending: bool = ((v_i & v_size) == 0i);
                var v_a: u32 = s_keys[v_i];
                var v_b: u32 = s_keys[v_other];
                var v_ia: u32 = s_ids[v_i];
                var v_ib: u32 = s_ids[v_other];
                var v_greater: bool = ((v_a > v_b) || ((v_a == v_b) && (v_ia > v_ib)));
                if (((v_ascending && v_greater) || ((!v_ascending) && (!v_greater)))) {
                  s_keys[v_i] = v_b;
                  s_keys[v_other] = v_a;
                  s_ids[v_i] = v_ib;
                  s_ids[v_other] = v_ia;
  ...
  if (v_valid) {
    {
      ...
      loop {
        if (!(v_r < (2048i / 256i))) { break; }
        var v_i: i32 = (v_lane + (v_r * 256i));
        var v_id: i32 = ((v_slot * 2048i) + i32(s_ids[v_i]));
        b_Order[((v_slot * 2048i) + v_i)] = v_id;
        var v_b: i32 = (v_id * 16i);
        var v_n: i32 = ((((v_slot * 4096i) + 2048i) + v_i) * 8i);
        var v_lo: vec3<f32> = vec3<f32>(1000000.0f, 1000000.0f, 1000000.0f);
        var v_hi: vec3<f32> = vec3<f32>((-1000000.0f), (-1000000.0f), (-1000000.0f));
        if ((b_P[(v_b + 3i)] >= 0.0f)) {
          let cw_argument_index_7 = v_b;
          let cw_argument_index_8 = (v_b + 1i);
          let cw_argument_index_9 = (v_b + 2i);
          var v_p: vec3<f32> = vec3<f32>(b_P[cw_argument_index_7], b_P[cw_argument_index_8], b_P[cw_argument_index_9]);
          let cw_argument_index_10 = (v_b + 4i);
          let cw_argument_index_11 = (v_b + 5i);
          let cw_argument_index_12 = (v_b + 6i);
          var v_h: vec3<f32> = vec3<f32>(b_P[cw_argument_index_10], b_P[cw_argument_index_11], b_P[cw_argument_index_12]);
          if (((i32(b_P[(v_b + 8i)]) % 2i) == 1i)) {
            var v_tmp: f32 = v_h.x;
            v_h.x = v_h.z;
            v_h.z = v_tmp;
          }
          v_lo = (v_p - v_h);
          v_hi = (v_p + v_h);
          if (((i32(b_P[(v_b + 3i)]) == 3i) || (i32(b_P[(v_b + 3i)]) == 4i))) {
            v_lo.y = v_p.y;
          }
          v_lo = (v_lo - vec3<f32>(0.008f, 0.008f, 0.008f));
          v_hi = (v_hi + vec3<f32>(0.008f, 0.008f, 0.008f));
        }
        b_Nodes[v_n] = v_lo.x;
        b_Nodes[(v_n + 1i)] = v_lo.y;
        b_Nodes[(v_n + 2i)] = v_lo.z;
        var cw_tmp_13: f32;
        if ((b_P[(v_b + 3i)] >= 0.0f)) {
          cw_tmp_13 = 1.0f;
        } else {
        ...
        b_Nodes[(v_n + 3i)] = cw_tmp_13;
        b_Nodes[(v_n + 4i)] = v_hi.x;
        b_Nodes[(v_n + 5i)] = v_hi.y;
        b_Nodes[(v_n + 6i)] = v_hi.z;
        b_Nodes[(v_n + 7i)] = 0.0f;
        continuing {
          v_r += i32(1);
        }
