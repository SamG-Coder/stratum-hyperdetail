// CUDA WebShader 0.1.0. Generated from kernel materialGradient.
...
@group(0) @binding(3) var<storage, read_write> b_Grad: array<f32>;
@group(0) @binding(4) var<storage, read_write> b_Brain: array<f32>;
...
fn main(
  ...
  if (((v_i >= 291i) || (!f_cw_buffer_helper_0(0i, cw_thread, cw_block, cw_grid)))) {
    return;
  }
  var v_g: f32 = 0.0f;
  var v_count: f32 = 0.0f;
  {
    var v_n: i32 = 0i;
    loop {
      if (!(v_n < 56i)) { break; }
      ...
      if ((b_Work[(v_b + 62i)] > 0.5f)) {
        ...
        if ((v_i < 216i)) {
          ...
          var v_h: i32 = cw_tmp_18;
          var v_d: f32 = 0.0f;
          {
            var v_o: i32 = 0i;
            loop {
              if (!(v_o < 3i)) { break; }
              v_d = (v_d + ((b_Work[((v_b + 11i) + v_o)] - b_Work[((v_b + 8i) + v_o)]) * b_W[((216i + (v_o * 24i)) + v_h)]));
              continuing {
                v_o += i32(1);
              }
            }
          }
          var v_a: f32 = b_Work[((v_b + 14i) + v_h)];
          v_d = (v_d * (1.0f - (v_a * v_a)));
          var cw_tmp_19: f32;
          if ((v_i < 192i)) {
            cw_tmp_19 = (v_d * b_Work[(v_b + (v_i % 8i))]);
          } else {
            cw_tmp_19 = v_d;
          }
          v_g = (v_g + cw_tmp_19);
          ...
          if ((v_i < 288i)) {
            var v_o: i32 = ((v_i - 216i) / 24i);
            var v_h: i32 = ((v_i - 216i) % 24i);
            v_g = (v_g + ((b_Work[((v_b + 11i) + v_o)] - b_Work[((v_b + 8i) + v_o)]) * b_Work[((v_b + 14i) + v_h)]));
          } else {
            var v_o: i32 = (v_i - 288i);
            v_g = (v_g + (b_Work[((v_b + 11i) + v_o)] - b_Work[((v_b + 8i) + v_o)]));
          }
        }
      }
      continuing {
  ...
  b_Grad[v_i] = f_clampf(cw_divide_f32(v_g, max(1.0f, v_count)), (-0.8f), 0.8f, cw_thread, cw_block, cw_grid);
  if ((v_i == 0i)) {
    if ((v_count > 0.0f)) {
      b_Brain[0i] = (b_Brain[0i] + 1.0f);
    }
    b_Brain[5i] = v_count;
  }
}
