// CUDA WebShader 0.1.0. Generated from kernel initWorld.
@group(0) @binding(0) var<storage, read_write> b_World: array<f32>;
@group(0) @binding(1) var<storage, read_write> b_Meta: array<f32>;
...
fn main(
  ...
  if ((v_i < (64i * 64i))) {
    var v_cx: i32 = ((v_i % 64i) - (64i / 2i));
    var v_cz: i32 = ((v_i / 64i) - (64i / 2i));
    var v_b: i32 = (v_i * 8i);
    var v_r: f32 = f_hash2(v_cx, v_cz, cw_params.p_seed, cw_thread, cw_block, cw_grid);
    var cw_tmp_7: i32;
    if ((v_r > 0.95f)) {
      cw_tmp_7 = 1i;
    } else {
    ...
    if (((v_cx == 0i) && ((v_cz == 1i) || (v_cz == 2i)))) {
      v_type = 3i;
    }
    var v_floors: i32 = (3i + i32((f_hash2((v_cx + 19i), v_cz, cw_params.p_seed, cw_thread, cw_block, cw_grid) * 4.0f)));
    var v_h: f32 = (4.2f + (f32(v_floors) * 3.8f));
    if ((v_type == 2i)) {
      v_h = 30.8f;
    }
    b_World[v_b] = (11.0f + (f_hash2((v_cx + 31i), v_cz, cw_params.p_seed, cw_thread, cw_block, cw_grid) * 3.0f));
    b_World[(v_b + 1i)] = (10.5f + (f_hash2(v_cx, (v_cz + 81i), cw_params.p_seed, cw_thread, cw_block, cw_grid) * 3.5f));
    b_World[(v_b + 2i)] = v_h;
    b_World[(v_b + 3i)] = f32(v_type);
    var cw_tmp_8: f32;
    if ((v_r > 0.52f)) {
      cw_tmp_8 = 1.0f;
    } else {
    ...
    b_World[(v_b + 4i)] = cw_tmp_8;
    b_World[(v_b + 5i)] = f32(v_floors);
    b_World[(v_b + 6i)] = f32((f_hashU(u32((((v_cx * 1973i) + (v_cz * 9277i)) + cw_params.p_seed)), cw_thread, cw_block, cw_grid) & 65535u));
    b_World[(v_b + 7i)] = v_r;
  }
  if ((v_i < 256i)) {
    {
      ...
      loop {
        if (!(v_j < 12i)) { break; }
        b_Meta[((v_i * 12i) + v_j)] = 0.0f;
        continuing {
          v_j += i32(1);
        }
      }
    }
    b_Meta[((v_i * 12i) + 3i)] = 0.0f;
  }
  if ((v_i == 0i)) {
    {
      var v_j: i32 = 0i;
