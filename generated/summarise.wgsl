// CUDA WebShader 0.1.0. Generated from kernel summarise.
@group(0) @binding(0) var<storage, read> b_Meta: array<f32>;
@group(0) @binding(1) var<storage, read> b_Req: array<f32>;
@group(0) @binding(2) var<storage, read> b_C: array<f32>;
@group(0) @binding(3) var<storage, read_write> b_Stats: array<f32>;
...
fn main(
  ...
  var v_resident: i32 = 0i;
  var v_prims: i32 = 0i;
  var v_visible: i32 = 0i;
  var v_reused: i32 = 0i;
  var v_wanted: i32 = 0i;
  var v_clusters: i32 = 0i;
  {
    var v_s: i32 = 0i;
    loop {
      if (!(v_s < 256i)) { break; }
      var v_b: i32 = (v_s * 12i);
      if ((b_Meta[(v_b + 3i)] > 0.5f)) {
        v_resident += i32(1);
        v_prims = (v_prims + i32(b_Meta[(v_b + 4i)]));
      }
      if ((b_Req[((v_s * 8i) + 2i)] > 0.0f)) {
        v_wanted += i32(1);
        var v_same: bool = (((i32(b_Meta[v_b]) == i32(b_Req[(v_s * 8i)])) && (i32(b_Meta[(v_b + 1i)]) == i32(b_Req[((v_s * 8i) + 1i)]))) && (b_Meta[(v_b + 3i)] > 0.5f));
        if (v_same) {
          v_visible += i32(1);
          if ((b_Meta[(v_b + 5i)] != b_C[6i])) {
            v_reused += i32(1);
          }
          v_clusters = (v_clusters + 64i);
        }
      }
      continuing {
        v_s += i32(1);
  ...
  b_Stats[0i] = f32(v_resident);
  b_Stats[3i] = f32(v_prims);
  b_Stats[4i] = f32(v_visible);
  b_Stats[5i] = f32(v_reused);
  b_Stats[6i] = f32(v_wanted);
  b_Stats[7i] = f32(v_clusters);
  b_Stats[8i] = (b_Stats[8i] + b_Stats[1i]);
}
