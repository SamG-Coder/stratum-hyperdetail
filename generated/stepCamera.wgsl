// CUDA WebShader 0.1.0. Generated from kernel stepCamera.
@group(0) @binding(0) var<storage, read_write> b_C: array<f32>;
@group(0) @binding(1) var<storage, read> b_I: array<f32>;
@group(0) @binding(2) var<storage, read> b_World: array<f32>;
struct CWParams {
  p_dt: f32,
  p_width: i32,
  p_height: i32,
...
fn main(
  ...
  b_C[5i] = (b_C[5i] + cw_params.p_dt);
  b_C[6i] = (b_C[6i] + 1.0f);
  b_C[13i] = f32(cw_params.p_height);
  b_C[20i] = cw_divide_f32(f32(cw_params.p_width), f32(cw_params.p_height));
  b_C[15i] = 0.0f;
  var v_bookmark: i32 = i32(b_I[8i]);
  if ((v_bookmark > 0i)) {
    if ((v_bookmark == 1i)) {
      b_C[0i] = 112.0f;
      b_C[1i] = 76.0f;
    ...
    if ((v_bookmark == 3i)) {
      b_C[0i] = (b_World[(f_worldIndex(0i, 0i, cw_thread, cw_block, cw_grid) * 8i)] + 1.1f);
      b_C[1i] = 6.9f;
      b_C[2i] = 1.2f;
      b_C[3i] = ((-3.141592653589793f) * 0.5f);
      b_C[4i] = 0.015f;
      b_C[12i] = 0.8f;
    }
    if ((v_bookmark == 4i)) {
    ...
    if ((v_bookmark == 6i)) {
      b_C[0i] = (b_World[(f_worldIndex(0i, 0i, cw_thread, cw_block, cw_grid) * 8i)] + 0.18f);
      b_C[1i] = 6.18f;
      b_C[2i] = 1.2f;
      b_C[3i] = ((-3.141592653589793f) * 0.5f);
      b_C[4i] = 0.0f;
  ...
  if (((b_I[3i] != 0.0f) || (b_I[4i] != 0.0f))) {
    b_C[3i] = (b_C[3i] + (b_I[3i] * 0.0022f));
    b_C[4i] = f_clampf((b_C[4i] - (b_I[4i] * 0.0022f)), (-1.53f), 1.53f, cw_thread, cw_block, cw_grid);
    b_C[15i] = 1.0f;
  }
  if ((b_I[7i] != 0.0f)) {
    b_C[12i] = f_clampf((b_C[12i] * exp(((-b_I[7i]) * 0.0015f))), 0.03f, 1600.0f, cw_thread, cw_block, cw_grid);
  }
  var cw_tmp_7: f32;
  if ((b_I[5i] > 0.5f)) {
    cw_tmp_7 = 5.0f;
  ...
  var v_speed: f32 = ((b_C[12i] * cw_tmp_7) * cw_tmp_8);
  var v_f: vec3<f32> = f_cw_buffer_helper_0(0i, cw_thread, cw_block, cw_grid);
  var v_r: vec3<f32> = f_cw_buffer_helper_1(0i, cw_thread, cw_block, cw_grid);
  let cw_argument_index_9 = 2i;
  var v_move: vec3<f32> = (((v_f * vec3<f32>(b_I[0i])) + (v_r * vec3<f32>(b_I[1i]))) + vec3<f32>(0.0f, b_I[cw_argument_index_9], 0.0f));
  if ((f_dot3(v_move, v_move, cw_thread, cw_block, cw_grid) > 0.0f)) {
    v_move = ((f_norm3(v_move, cw_thread, cw_block, cw_grid) * vec3<f32>(v_speed)) * vec3<f32>(cw_params.p_dt));
    b_C[0i] = (b_C[0i] + v_move.x);
    b_C[1i] = max(0.08f, (b_C[1i] + v_move.y));
    b_C[2i] = (b_C[2i] + v_move.z);
    b_C[15i] = 1.0f;
  }
  if ((b_I[9i] != 0.0f)) {
    b_C[7i] = (b_C[7i] + ((b_I[9i] * cw_params.p_dt) * 0.35f));
    b_C[15i] = 1.0f;
  }
  if ((b_I[10i] > 0.0f)) {
    b_C[11i] = f32((i32(b_C[11i]) + 1i));
    if ((b_C[11i] > 3.0f)) {
      b_C[11i] = 0.0f;
    }
    b_C[15i] = 1.0f;
  ...
  if ((b_I[11i] > 0.0f)) {
    b_C[16i] = (1.0f - b_C[16i]);
    b_C[15i] = 1.0f;
  }
  if ((b_I[12i] > 0.0f)) {
    b_C[17i] = (1.0f - b_C[17i]);
    b_C[15i] = 1.0f;
  }
  if ((b_I[13i] != 0.0f)) {
    b_C[9i] = f_clampf((b_C[9i] + (b_I[13i] * cw_params.p_dt)), 0.25f, 3.0f, cw_thread, cw_block, cw_grid);
    b_C[15i] = 1.0f;
  }
  if ((b_I[14i] > 0.0f)) {
    b_C[18i] = (1.0f - b_C[18i]);
    b_C[15i] = 1.0f;
  }
  if ((b_I[15i] > 0.0f)) {
    b_C[21i] = (1.0f - b_C[21i]);
  }
  if ((b_I[16i] > 0.0f)) {
    b_C[22i] = (1.0f - b_C[22i]);
  }
  if ((b_C[22i] > 0.5f)) {
    var v_a: f32 = (b_C[5i] * 0.025f);
    b_C[0i] = (cos(v_a) * 150.0f);
    b_C[2i] = (sin(v_a) * 150.0f);
    b_C[1i] = (58.0f + (15.0f * sin((v_a * 0.7f))));
    b_C[3i] = (((-3.141592653589793f) * 0.5f) - v_a);
    b_C[4i] = (-0.24f);
    b_C[15i] = 1.0f;
  }
  var cw_tmp_10: f32;
  if ((b_C[15i] > 0.5f)) {
    ...
    cw_tmp_10 = min(32.0f, (b_C[19i] + 1.0f));
  }
  b_C[19i] = cw_tmp_10;
}
