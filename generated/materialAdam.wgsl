// CUDA WebShader 0.1.0. Generated from kernel materialAdam.
@group(0) @binding(0) var<storage, read> b_C: array<f32>;
@group(0) @binding(1) var<storage, read> b_Grad: array<f32>;
@group(0) @binding(2) var<storage, read_write> b_W: array<f32>;
@group(0) @binding(3) var<storage, read_write> b_M: array<f32>;
@group(0) @binding(4) var<storage, read_write> b_V: array<f32>;
...
fn main(
  ...
  if ((((v_i >= 291i) || (!f_cw_buffer_helper_0(0i, cw_thread, cw_block, cw_grid))) || (b_Brain[5i] < 1.0f))) {
    return;
  }
  var v_g: f32 = b_Grad[v_i];
  var v_m: f32 = ((0.9f * b_M[v_i]) + (0.1f * v_g));
  var v_v: f32 = ((0.999f * b_V[v_i]) + ((0.001f * v_g) * v_g));
  b_M[v_i] = v_m;
  b_V[v_i] = v_v;
  let cw_argument_index_18 = 0i;
  var v_mh: f32 = cw_divide_f32(v_m, (1.0f - cw_pow_f32(0.9f, b_Brain[cw_argument_index_18])));
  let cw_argument_index_19 = 0i;
  var v_vh: f32 = cw_divide_f32(v_v, (1.0f - cw_pow_f32(0.999f, b_Brain[cw_argument_index_19])));
  b_W[v_i] = (b_W[v_i] - (0.002f * (cw_divide_f32(v_mh, (sqrt(v_vh) + 0.00001f)) + (0.0001f * b_W[v_i]))));
}
