...
__global__ void generatePages(const float* World,const float* Req,const int* Queue,float* P){
 ...
 if(group>=2&&group<=5&&lod>=4){
  ...
  for(int k=0;k<4;k++){float yy=5.05f+(float)k*fmaxf(1.0f,(h-6.0f)/4.0f);
   n=wallBox(P,base,n,face,x,z,w,d,0.0f,yy,0.205f,ext+0.08f,0.035f,0.055f,13,seed+410.0f+(float)k);}
  if(lod>=5)for(int k=0;k<4;k++){float u=((float)k-1.5f)*(ext*0.48f);
   n=wallBox(P,base,n,face,x,z,w,d,u,h-0.72f,0.255f,0.055f,0.34f,0.07f,0,seed+450.0f+(float)k);}
 }
 if(group>=6&&group<30&&lod>=4){
  int face=(group-6)/6;int row=(group-6)%6;if(row<floors){float ext=face%2==0?w:d;float y=6.1f+(float)row*3.8f;
   for(int j=0;j<4;j++){float u=((float)j-1.5f)*(ext*0.48f);
    ...
    n=wallBox(P,base,n,face,x,z,w,d,u-0.46f,y,0.145f,0.026f,1.10f,0.055f,5,seed+500.0f+(float)j);
    n=wallBox(P,base,n,face,x,z,w,d,u+0.46f,y,0.145f,0.026f,1.10f,0.055f,5,seed+520.0f+(float)j);
   }
   if(lod>=5)for(int j=0;j<4;j++){float u=((float)j-1.5f)*(ext*0.48f);
    ...
    n=wallBox(P,base,n,face,x,z,w,d,u,y-1.31f,0.30f,1.18f,0.045f,0.07f,13,seed+540.0f+(float)j);
    n=wallBox(P,base,n,face,x,z,w,d,u,y+1.31f,0.18f,1.08f,0.038f,0.055f,13,seed+560.0f+(float)j);
   }
  }
 }
 if(group>=32&&group<56&&lod>=4){
  int face=(group-32)/6;int row=(group-32)%6;if(row<floors){float ext=face%2==0?w:d;float y=6.1f+(float)row*3.8f;
   for(int j=0;j<4;j++){float u=((float)j-1.5f)*(ext*0.48f);
    ...
    n=wallBox(P,base,n,face,x,z,w,d,u,y-0.22f,1.17f,1.20f,0.028f,0.035f,6,seed+600.0f+(float)j);
    n=wallBox(P,base,n,face,x,z,w,d,u,y-1.23f,0.62f,1.19f,0.038f,0.038f,6,seed+620.0f+(float)j);
   }
   if(lod>=5)for(int j=0;j<4;j++){float u=((float)j-1.5f)*(ext*0.48f);
    // Balcony brackets become actual geometry rather than a normal-map cue.
    n=wallBox(P,base,n,face,x,z,w,d,u-0.72f,y-1.42f,0.46f,0.075f,0.22f,0.42f,0,seed+640.0f+(float)j);
    n=wallBox(P,base,n,face,x,z,w,d,u+0.72f,y-1.42f,0.46f,0.075f,0.22f,0.42f,0,seed+660.0f+(float)j);
   }
  }
 }
 if(group==30&&lod>=4){
  ...
  for(int k=0;k<4;k++){float side=k<2?-1.0f:1.0f;float along=(k%2==0?-0.58f:0.58f);
   n=emit(P,base,n,make_float3(x+side*(w+0.34f),h*0.38f,z+along*d),make_float3(0.075f,h*0.38f,0.075f),2,6,0,seed+700.0f+(float)k);}
  for(int k=0;k<4;k++)n=emit(P,base,n,make_float3(x-12.0f+(float)k*8.0f,0.48f,z+d+2.0f),make_float3(0.12f,0.48f,0.12f),2,6,0,seed+720.0f+(float)k);
  if(lod>=5)for(int k=0;k<8;k++)n=emit(P,base,n,make_float3(x-14.0f+(float)k*4.0f,0.055f,z-d-2.2f),make_float3(1.75f,0.035f,0.045f),0,13,0,seed+740.0f+(float)k);
 }
 if(group==31&&lod>=4){
  ...
  for(int k=0;k<4;k++){float xx=x+((float)(k%2)-0.5f)*w;float zz=z+((float)(k/2)-0.5f)*d;
   n=emit(P,base,n,make_float3(xx,h+0.72f,zz),make_float3(0.34f,0.48f,0.34f),2,6,0,seed+800.0f+(float)k);
   n=emit(P,base,n,make_float3(xx,h+1.18f,zz),make_float3(0.46f,0.055f,0.46f),0,13,0,seed+820.0f+(float)k);}
  if(lod>=5)for(int k=0;k<8;k++){float a=(float)k*PI*0.25f;
   n=emit(P,base,n,make_float3(x+cosf(a)*w*0.44f,h+0.42f,z+sinf(a)*d*0.44f),make_float3(0.055f,0.42f,0.055f),2,6,0,seed+840.0f+(float)k);}
 }
